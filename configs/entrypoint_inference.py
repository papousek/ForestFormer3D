...
score_th = 00.4
chunk = 20_000
num_points = 640_000  # modify the number of points fed into the model during training and testing
model = dict(
    ...
    criterion=dict(
        ...
        sem_criterion=dict(type="S3DISSemanticCriterion", loss_weight=0.2),
        inst_criterion=dict(
...
train_pipeline = [
    ...
    dict(type="PointSample_", num_points=640000),
...
metric_meta = dict(
    label2cat=label2cat, ignore_index=[], classes=class_names, dataset_name="ForAINetV2"
)
...
default_hooks = dict(
    checkpoint=dict(
        type="CheckpointHook", interval=1, max_keep_ckpts=3, save_optimizer=True
    ),
...
train_cfg = dict(type="EpochBasedTrainLoop", max_epochs=3000, val_interval=100)
